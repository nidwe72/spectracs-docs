...
> | **2** | **domain objects for product line / raw-material lot / production batch** — ⭐ **and a PRODUCER field** | ⭐⭐⭐ ESSENTIAL | ⭐ **decided 2026-09-01** — objects, not free text, and **oil-agnostic**: Edwin, *"even other oils have the same concepts/usages"*. ⭐⭐ **Second reason found 2026-09-02, and it is a live one: the archive carries "Spar Premium g.g.A." — the RETAIL BRAND — and no producer at all. That oil has been measured repeatedly, ranked, and quoted in three specs, and we did not know who pressed it** (it is the Silly family, learned from Edwin, not from any record of ours). ⇒ the item is now a **precondition** for the history tracker, for the verdict-free conformity check, and for any published per-batch value | §0c |
...
> | **9b** | ⭐ **Kreft's dichromaticity index on the EXISTING corpus** — `A × 4` by Beer–Lambert gives the fourfold-path colour from the same spectrum; take the CIELAB hue-angle difference | ⭐ **one afternoon, desk** | pumpkin seed oil is the **textbook example** of dichromatism and the index is **published** (Kreft & Kreft, JOSA A 2009). A metric with a **foreign name, defined before our question existed**, is worth more in the M9 discussion than another of our own — and if it does not separate green from brown, that is learned for nothing. ⛔ **pre-register first**; ⛔ and a **literature-comparable** DI needs the red extension (x̄ runs past 700 nm, our window ends at 630) — what is computable today is an internally consistent DI-**like** quantity, and it must be named that way | `SPEC_metric_research.md` §16.19 |
> | **9c** | ⭐⭐⭐ **the pumpkin-oil dilution ladder** — pumpkin seed oil in **sunflower oil** at 100/50/30/20/10/5 %, existing rig, frozen protocol, `k` fills per level; record **`Rv` AND a level term** | ⭐⭐⭐ **one evening, everything already in the house** | ⭐⭐⭐ **promoted over 9e on 2026-09-02, and it is the better experiment on six counts**: both materials are here (sunflower *is* the solvent), the archive is 98 runs deep, `Rv` is validated with `T = 52`, the sunflower arm already exists (`SPEC_metric_research.md` §14), the fraud it imitates is **documented** — *"sunflower oil simply coloured with a little pumpkin seed oil"* — and the customer for the answer is an hour's drive away. ⭐ **The expected result is predictable and that is why it is a good test:** `Rv` should stay **flat** (scale-invariant), the level should fall **linearly**. ⛔ **If `Rv` moves, the invariance claim in `DOC_metric_algebra.md` is wrong** — a finding about the metric, not about the fraud | `SPEC_history_tracker.md` §8.1 |

> | **9d** | ⚠ **does the sensor path pass 710 nm?** one grab through a 710 filter or at a 710 source | ⭐ **one grab** | it decides whether the lamp rebuild is *a better pumpkin instrument* (410–680) or *one that can execute a published norm* — **AOCS Cc 13i-96** needs 630/670/**710**. ⛔ **Must not delay the R2 order**: if 710 needs a sensor change that is a separate project, and R2 stands on the **six instrument-side arguments** of `SPEC_lamp_rebuild.md` §13.2 alone | `DOC_lamp_410_680.md` §11 |
> | **9e** | ⚠ **the walnut adulteration series** — walnut + sunflower at 0/5/10/20/30/50 %, `D` and the level term | ⚠ one evening, ~20 € | ⛔ **DEMOTED 2026-09-02, was 9c.** Still the right experiment for a *second* oil, and it has a published expectation (420–500 nm after SNV, 5–50 %, 70–95 %). But **9c answers the same question on home ground** with a validated metric and an archive behind it, and a null result there is far easier to interpret. ⇒ **run 9e only if 9c succeeds and a second oil is actually wanted** | `SPEC_history_tracker.md` §7 |
> | **9f** | ⭐⭐⭐ **the in-house COLOUR ranking — 8–12 oils, all at 2 capillaries in 8 ml, light table, blinded, Edwin + 2–3 LAY observers, ties allowed** | ⭐⭐⭐ **one afternoon — and it has a DATE** | ⭐⭐⭐ **there is no colour-specific human ranking of this archive from more than one pair of eyes anywhere on record.** §16.17.2 has one, on two oils, and states its own weakness — that is the entire human-side evidence base for a metric whose verdict is about colour. §16.18's external ÖGVS ranking is stronger on provenance but its **limit 2** says it measures overall sensory quality, not colour. ⭐⭐ **The lay observers also measure the CEILING: if ρ(metric, human) ≥ ρ(human, human), the metric is already as good as a person.** ⛔ **Ties are mandatory** — "metric separates where the eye cannot" is the reason to own an instrument, and forced guessing hides it as random error. ⚠ Gated on Edwin's metric decision (~2 weeks) and needed **before the Wagner presentation** | `SPEC_metric_research.md` §16.21 |
> | **9g** | ⭐⭐ **measure the rig's LINE SHAPE — a fluorescent tube's Hg lines at 405 / 436 / 546 / 611 nm** | ⭐⭐ **one afternoon, free** | their width **is** the instrument's resolution. Two uses: it is the **minimum slit width to demand when buying** a lab spectrometer, and it is the kernel needed to **convolve a higher-resolution bought instrument down to this rig's bandpass** — which is what lets `Rv`, `T = 52` and the band edges transfer **by construction** instead of being refitted. ⛔ High-res → coarse always works; coarse → high-res never does | `KB_lamps.md`, `SPEC_oelmuehlen_verzeichnis.md` §155.4 |
> | **9h** | ⭐⭐ **the collinearity check — Spearman matrix over `A_Soret`, `A_valley`, `A_Q`, `A_568`, `A_624` and the derived `Q%`/`R`/`Rv` on the 98-run archive** | ⭐⭐ **one afternoon, desk, no rig time** | ⚠ **a published warning aimed at our exact algebraic form.** Hensel et al. 2022 showed the **Glories** absorbances (420/520/620 — fixed wavelengths, sums and ratios, i.e. our shape) correlate strongly **with each other**, *"in violation of Grassmann's first law"*, so that index carries **less information than it appears to**. If our bands are likewise collinear, our three metrics are **one** piece of information under three names and every "the second metric agrees" argument is circular. ⛔ **Pre-register the threshold before running it** | `SPEC_metric_research.md` §16.22 |
> | **9i** | ⭐ **a clear-glass window in front of the slit** — Baader *Klarglas 1,25"* (31.8 mm, 2 mm, multicoated, **€39**), as a **sacrificial** surface against bench dust, oil and IPA vapour | ⭐ **hygiene, not accuracy — decide, do not schedule** | the slit is the one part that **cannot be cleaned without disturbing alignment**. Coated it costs ≈1 % and, like the Wratten 85 and the jar's 0.859, **cancels in `T = S/R`**. ⛔ **But the objection is stray light, not loss:** a new reflective flat directly in front of the slit is a new ghost path, and §16.24 says the baseline is **62 % of raw `Q`** vs **−1 % of raw Soret** — the budget is 17× asymmetric and *pedestal is the direction it is weak in*. Tilting kills the ghost but displaces the beam laterally = a **pixel→nm shift**, so it must be tilted **and mechanically fixed**. ⛔ Ranks below the fill/session work (§16.44: instrument **1.0** vs fill **4.1**, session **4.8**) and guards a failure mode **nobody has yet observed**. ⭐ **The one thing that moves it up is TIMING: fitting it before the next calibration is cheap, after it is a recalibration** — so decide it *with* the lamp rebuild, which opens the optical path anyway | `SPEC_capture_quality.md` §16.45, `SPEC_lamp_rebuild.md` R2 |
> | **9-SHIP** | ⭐⭐⭐ **the oil-profile readings into the plugin + a final cleanup** — ● sentences only for millers, cut points as versioned plugin constants; cleanup in a separate session (verify in code, don't trust old notes: `Q%` on the verdict pill? stale sealed plugin row? old protocol strings?) | ⭐⭐⭐ **the step to shippable** | ⭐ **Edwin, 2026-09-19:** a specialist tool for expert millers with no alternative worldwide ⇒ **shippable once the readings are in and the cleanup is done**. `Rv` as chosen stands; > ~20 `Rv` separation suffices for now; the settled measurement works as is. Suggested first delivery: a parallel-measurement pilot with 1–2 millers | `SPEC_oil_profile_readings.md` §8.1 |
> | **9j** | ⭐⭐ **pre-register the oil-profile cut points and rules** — `Rv` / browning / sample darkness levels, the browning gate 0.75–0.95, rules R1–R9, and the predictions of 9k–9m | ⭐⭐ **desk, before 9k–9n are read** | the profile (2026-09-19) was built on the ten oils it is shown on; its cuts are **not** pre-registered and every rendering says so. ⛔ Writing the predictions down first is what keeps 9k–9n tests instead of fits | `SPEC_oil_profile_readings.md` §4–§5, §8 |
> | **9k** | ⭐⭐ **best-before dates of all ten suite bottles next to `Rv`** | ⭐⭐ **free, ten minutes** | Kiendler looks like `Rv` ≈ 80 and measures 63.6. Plausible mechanism: protopheophytin also forms during **seed storage**, which makes no Maillard browning ⇒ old seed, gently roasted = low `Rv`, green look. Prediction: Kiendler was pressed late in the season. ⭐ If the dates line up with `Rv` across the ten, seed age becomes a field the corpus needs (see item 2) | `SPEC_oil_profile_readings.md` §1, §8; `SPEC_metric_research.md` §16.23.7 |
> | **9l** | ⭐⭐ **the equal-absorbance photograph** — Kiendler prepared as dark as Esterer and Birnstingl (one capillary into **3 ml** instead of 4), photographed against both | ⭐⭐ **one photo session** | separates the two explanations of the Kiendler case: Kiendler falls back toward its `Rv` ⇒ the **paleness** of its sample was the cause (A448 0.68 vs 0.91–0.93); still looks ≈ 80 ⇒ the eye reads something `Rv` does not (the blue edge, or the red above 636 nm) | `SPEC_oil_profile_readings.md` §8 |
> | **9m** | ⭐⭐ **the two-dose fill** — each oil at 1 and 2 capillaries into 4 ml | ⭐⭐ **one evening** | the browning proxy `A500/A448` carries a constant offset that makes **pale samples read browner**; the pigment doubles with the dose, the offset does not ⇒ solvable per oil (the route `r_Q` was measured by on Kiendler, 2026-08-01). It **replaces the provisional 0.75–0.95 browning gate**, which today switches browning off for 5 of 10 oils — Kiendler included | `SPEC_oil_profile_readings.md` §4.1 |
> | **9n** | ⭐ **the photographic round robin of the six 09-14 oils** — all 15 pairs (minimum with a full transitivity check; 9 = chain + skip-one for a partial one; 5 = chain only, no check) | ⭐ **one photo session** | the eye check of 2026-09-18/19 was a visual inspection, not a photographic series like the four oils of the test-suite report. ⛔ **pick pairs by name, not by `Rv`**, or the design leaks the expected order. ⚠ The green/brown **class** labels of Birnstingl, Kiendler and Spar ggA are still owed and this does not supply them. Related to 9f | `SPEC_metric_research.md` §16.23.5, §16.23.7 |
