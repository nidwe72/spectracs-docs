...
> | **9c** | ⭐⭐ **the walnut adulteration series** — walnut oil + sunflower oil at 0/5/10/20/30/50 %, existing rig, frozen protocol, `k` fills per level; record **`D` AND the level term** | ⭐⭐ **one evening, ~20 €** | ⭐⭐⭐ **the cheapest decisive experiment on this page.** `SPEC_history_tracker.md` §4 names the risk that `D` degenerates to an amplitude echo on any oil whose bands sit outside 440–630; a published Vis-NIR result says 420–500 nm after SNV carries walnut discrimination at 5–50 % with 70–95 % accuracy — **so here the difference is known to exist and we set its size**, which makes a null result indict `D` rather than the oil. It would also be the first datasheet number this project has for any oil but pumpkin. ⭐ Lugitsch presses walnut **and is already in the measurement archive**. ⛔ Record the level term too — §8 shows a featureless diluent is invisible to `D` by construction | `SPEC_history_tracker.md` §7, §8 |
> | **9d** | ⚠ **does the sensor path pass 710 nm?** one grab through a 710 filter or at a 710 source | ⭐ **one grab** | it decides whether the lamp rebuild is *a better pumpkin instrument* (410–680) or *one that can execute a published norm* — **AOCS Cc 13i-96** needs 630/670/**710**. ⛔ **Must not delay the R2 order**: if 710 needs a sensor change that is a separate project, and R2 stands on its own eight arguments | `DOC_lamp_410_680.md` §11 |
